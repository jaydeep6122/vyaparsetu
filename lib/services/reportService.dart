...
class ReportService {
  static final _format = NumberFormat.decimalPattern('en_IN')
  ...
  static Future<void> generateBusinessHealthReport({
    ...
    final saleInvoices = invoices.where((i) => i.invoiceType == InvoiceType.sale).toList();
    final purchaseInvoices = invoices.where((i) => i.invoiceType == InvoiceType.purchase).toList();
    ...
    final purchaseTotal = purchaseInvoices.fold(0.0, (s, i) => s + i.totalAmount);
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          _keyValue('Net Profit / Loss', _f(netProfit), netProfit >= 0, bold, font),
          ...
          pw.SizedBox(height: 4),
          _keyValue('Outstanding', _f(saleTotal - saleReceived), false, bold, font),
          pw.SizedBox(height: 20),
          _sectionTitle('Purchase Metrics', bold),
          pw.SizedBox(height: 8),
          _keyValue('Base (excl. GST)', _f(purchaseBase), false, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('GST Amount', _f(purchaseTax), false, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('Total (incl. GST)', _f(purchaseTotal), false, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('Paid', _f(purchasePaid), true, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('Pending Payables', _f(purchaseTotal - purchasePaid), false, bold, font),
          pw.SizedBox(height: 20),
          _footer(bold, font),
        ],
      ),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.SizedBox(height: 16),
          _table(
            headers: ['Date', 'Invoice No', 'Party', 'Base', 'GST', 'Total', 'Paid', 'Status'],
          ...
          pw.SizedBox(height: 4),
          _keyValue('Outstanding', _f(totalAmount - totalReceived), false, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('Number of Invoices', '${sorted.length}', true, bold, font),
          pw.SizedBox(height: 20),
          _footer(bold, font),
        ],
      ),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.SizedBox(height: 16),
          _table(
            headers: ['Date', 'Invoice No', 'Supplier', 'Base', 'GST', 'Total', 'Paid', 'Status'],
            widths: const [60, 70, 110, 55, 55, 55, 55, 55],
            rows: sorted.map((i) => [
              _d(i.invoiceDate),
              i.invoiceNumber,
          ...
          pw.SizedBox(height: 4),
          _keyValue('Total (incl. GST)', _f(totalAmount), false, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('Paid', _f(totalPaid), true, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('Pending Payables', _f(totalAmount - totalPaid), false, bold, font),
          pw.SizedBox(height: 4),
          _keyValue('Number of Purchases', '${sorted.length}', true, bold, font),
          pw.SizedBox(height: 20),
          _footer(bold, font),
        ],
      ),
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.Text('Party: $partyName', style: pw.TextStyle(font: bold, fontSize: 12)),
          pw.SizedBox(height: 16),
          _table(
            headers: ['Date', 'Invoice No', 'Base', 'GST', 'Total', 'Paid', 'Status'],
  ...
  static pw.Widget _header(
    ...
    return pw.Column(
      ...
      children: [
        pw.Text(b.name, style: pw.TextStyle(font: bold, fontSize: 20, color: PdfColors.blue900)),
        pw.SizedBox(height: 4),
        if (b.address != null)
          pw.Text(b.address!, style: pw.TextStyle(font: font, fontSize: 9, color: PdfColors.grey700)),
        if (b.gstin != null && b.gstin!.isNotEmpty)
          pw.Text('GSTIN: ${b.gstin}', style: pw.TextStyle(font: font, fontSize: 9, color: PdfColors.grey700)),
        pw.SizedBox(height: 8),
        pw.Container(height: 1, color: PdfColors.grey400),
        pw.SizedBox(height: 8),
        pw.Text(title, style: pw.TextStyle(font: bold, fontSize: 16)),
        pw.Text('Period: $period', style: pw.TextStyle(font: font, fontSize: 10, color: PdfColors.grey700)),
        pw.SizedBox(height: 4),
      ],
    );
  }
