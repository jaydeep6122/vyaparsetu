...
context.read<Core>().payment.fetchPayments(
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: Text('Payment In', style: GoogleFonts.outfit(fontWeight: FontWeight.w600)),
                subtitle: Text('Record money received from a customer',
                  style: GoogleFonts.outfit(fontSize: 12, color: AppTheme.slate500)),
                onTap: () {
                  Navigator.of(ctx).pop();
                  Navigator.of(context).push(
                    getPageRoute(const PaymentFormScreen.paymentIn()),
                  ).then((_) {
              ...
              ListTile(
                leading: CircleAvatar(
                  ...
                  child: const Icon(Icons.payments_outlined, color: AppTheme.error),
                ),
                title: Text('Payment Out', style: GoogleFonts.outfit(fontWeight: FontWeight.w600)),
                subtitle: Text('Record payment made to a supplier',
                  style: GoogleFonts.outfit(fontSize: 12, color: AppTheme.slate500)),
                onTap: () {
                  Navigator.of(ctx).pop();
                  Navigator.of(context).push(
                    getPageRoute(const PaymentFormScreen.paymentOut()),
                  ).then((_) {
                    if (businessId != null && mounted) _loadData();
                  });
                },
  ...
  Widget build(BuildContext context) {
    ...
    final payments = context.select<Core, List<Payment>>((c) => c.payment.payments);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          Padding(
              ...
              children: [
                Row(
                  children: [
                    ...
                    const Spacer(),
                    if (_filterType != null || _fromDate != null || _toDate != null)
                      GestureDetector(
                        onTap: _clearFilters,
                        child: Text(
                          'Clear',
            ...
            fontWeight: FontWeight.bold,
                ...
                Row(
                  children: [
                    Expanded(
                      child: _buildDateButton('From', _fromDate, () => _pickDate(true)),
                    ),
                    const SizedBox(width: 8),
                    Expanded(
                      child: _buildDateButton('To', _toDate, () => _pickDate(false)),
                    ),
                  ],
                ),
              ],
          ...
          Expanded(
            child: _buildPaymentList(isDark, theme, isLoading, payments, error, businessId),
          ),
        ],
      ),
  ...
  Widget _buildPaymentList(bool isDark, ThemeData theme, bool isLoading, List<Payment> payments, String? error, String? businessId) {
    if (isLoading && payments.isEmpty) {
      return const LoadingIndicator(message: 'Loading payments...', isShimmer: true);
    }
  ...
  Widget _buildFilterChip(String label, String? value) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? (isDark ? Colors.white.withValues(alpha: 0.2) : AppTheme.primary)
        ...
        child: Text(
          ...
          style: GoogleFonts.outfit(
            fontSize: 13,
            fontWeight: FontWeight.bold,
            color: isSelected ? Colors.white : theme.textTheme.bodyLarge?.color?.withValues(alpha: 0.7),
          ),
        ),
      ),
    );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(AppTheme.radiusSm),
          border: date != null ? Border.all(color: isDark ? Colors.white.withValues(alpha: 0.5) : AppTheme.primary) : null,
        ),
        child: Row(
          ...
          children: [
            Icon(
              ...
              size: 14,
              color: date != null ? (isDark ? Colors.white : AppTheme.primary) : AppTheme.gray400,
            ),
            ...
            Text(
              ...
              style: GoogleFonts.outfit(
                ...
                color: date != null ? (isDark ? Colors.white : AppTheme.primary) : null,
              ),
            ),
            if (date != null) ...[
              const SizedBox(width: 4),
